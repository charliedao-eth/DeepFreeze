...
interface IFactory {
    ...
    function stakingFRZ() external view returns (address);
}
...
contract DeepFreeze {
    ...
    // create a state for withdraw to avoid reentrancy
    ...
    function payEthPenalty() private {
        uint256 ethFees = IFactory(address(this)).getFees();
        address _stakingFRZ = IFactory(factoryAddress).stakingFRZ();
